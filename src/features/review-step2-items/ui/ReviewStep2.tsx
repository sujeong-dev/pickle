...
type ReviewStep2Props = {
  items: EditableItem[];
  onUpdateItem: (idx: number, name: string, price: number) => void;
};
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-1">
            {items.map((item, idx) => (
              <button
                ...
                className="flex items-center gap-1 border-b border-gray-100 py-2 w-full text-left"
              >
      ...
      {editIdx !== null && items[editIdx] && (
        <EditModal
          item={items[editIdx]}
          onSave={(name, price) => onUpdateItem(editIdx, name, price)}
